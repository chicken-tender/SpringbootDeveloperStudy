...
class BlogApiControllerTest {
  ...
  @Autowired
  protected ObjectMapper objectMapper; // JSON 데이터로 변환하는 직렬화, 역직렬화를 위한 클래스
  ...
  public void addArticle() throws Exception {
    ...
    ResultActions result = mockMvc.perform(post(url)
        .contentType(MediaType.APPLICATION_JSON) // 컨텐츠 타입 설정
        .content(requestBody)); // 요청의 내용 설정
    ...
    assertThat(articles.size()).isEqualTo(1); // 크기가 1인지 집중
  ...
  @DisplayName("findArticle : 블로그 특정 id 글 조회에 성공한다.")
  @Test
  ...
  @DisplayName("deleteArticle : 블로그 특정 글 삭제에 성공한다.")
  @Test
  ...
  @DisplayName("updateArticle : 블로그 특정 글 수정에 성공한다.")
  @Test
